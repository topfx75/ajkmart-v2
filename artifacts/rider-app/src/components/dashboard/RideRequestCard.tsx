...
export function RideRequestCard({
  ...
  const getMinFare = () => {
    ...
    if (vt === "car") return config.rides.carMinFare ?? 80;
    if (vt === "rickshaw") return config.rides.rickshawMinFare ?? 50;
    if (vt === "daba") return config.rides.dabaMinFare ?? 60;
    return config.rides.bikeMinFare ?? 50;
  };

  const getMaxFare = () => {
    return (r.offeredFare ?? r.fare) * (config.rides.counterMaxMultiplier ?? 3);
  };
  ...
  return (
    <div
      className={`p-4 animate-[slideUp_0.3s_ease-out] ${isDispatched ? "border-l-4 border-blue-500 bg-gradient-to-r from-blue-50/50 to-white" : isBargain ? "border-l-4 border-orange-400 bg-gradient-to-r from-orange-50/50 to-white" : "hover:bg-gray-50/50"} transition-colors`}
    >
      <div className="flex items-start gap-3">
        <AcceptCountdown createdAt={r.createdAt} onExpired={() => onDismiss(r.id)} />
        <div
          className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-sm border ${isDispatched ? "bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200" : isBargain ? "bg-gradient-to-br from-orange-50 to-amber-50 border-orange-200" : "bg-gradient-to-br from-green-50 to-emerald-50 border-green-100"}`}
        >
          {isBargain ? (
          ...
          {r.myBid ? (
            <div className="bg-gradient-to-r from-orange-50 to-amber-50 border-2 border-orange-200 rounded-xl p-3.5 space-y-2.5">
              ...
              <div className="flex gap-2">
                ...
                />
                {counterError && (
                  <p className="text-xs text-red-500 font-semibold col-span-full">
                    {counterError}
                  </p>
                )}
                <button
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="bg-gray-100 text-gray-400 font-bold px-3 py-2.5 rounded-xl text-sm flex items-center hover:bg-gray-200 transition-colors disabled:opacity-50"
